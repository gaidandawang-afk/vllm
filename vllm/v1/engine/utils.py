...
class FFNActorManager(BaseActorManager):
    def __init__(
        ...
        self.ep_master_ip = ep_master_ip
    ...
    def scale_up_elastic_ep(
        ...
        self.ffn_size = new_data_parallel_size
        ...
        for scaled_actor in self.actors[-(len(placement_groups)):]:
            self.run_refs.append(scaled_actor.run.remote())
        logger.info(f"FFN manager scale finished, {len(self.actors[-(len(placement_groups)):])} actors added.")
    ...
    def get_identities(self):
        ...
        for ffn_rank in range(self.ffn_size):
            identities.append(f"FFN_{ffn_rank}".encode("utf-8"))
        return identities
...
class GlobalActorManager(BaseActorManager):
    def __init__(
        ...
        manager_classes = [CoreEngineActorManager]
        if vllm_config.afd_config:
            manager_classes.append(FFNActorManager)

        from concurrent.futures import ThreadPoolExecutor, as_completed
        with ThreadPoolExecutor(max_workers=len(manager_classes)) as pool:
            futures = {}
...
class CoreEngineActorManager(BaseActorManager):
    ...
    def get_actor_class(self, vllm_config):
        from vllm.v1.engine.core import DPEngineCoreActor, DPMoEEngineCoreActor
        return  (
...
def launch_core_engines(
    ...
    if vllm_config.fault_tolerance_config.enable_fault_tolerance is True:
        ...
        num_identities = dp_size
        if vllm_config.afd_config:
            import re
            afd_size = vllm_config.afd_config.afd_extra_config.get("afd_size")
            attn_size, ffn_size = map(int, re.match(r"(\d+)\D+(\d+)", afd_size).groups())
            assert attn_size == dp_size, f"DP size {dp_size} must equal to attn_size {attn_size}"
            num_identities = dp_size + ffn_size
...
def wait_for_instruction_result(
    ...
    while target_identities:
        ...
        if remaining <= 0:
            logger.error(
                '%s Timeout while waiting for responses of command "%s" '
                "from identities: %s",
                timeout,
                method_name,
                target_identities,
